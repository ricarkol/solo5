...
cp /usr/lib/SYSLINUX/mbr.bin ${TMPDIR}/image.mbr || die "could not copy mbr"
...
sfdisk -o gpt -d ${TMPDIR}/image.mbr 
O_SECTORS=$(sfdisk -d ${TMPDIR}/image.mbr | grep start | awk '{print $4}' | sed -e s/,//)
# Size of partition (sectors)
S_SECTORS=$(sfdisk -d ${TMPDIR}/image.mbr | grep start | awk '{print $6}' | sed -e s/,//)
echo $O_SECTORS
echo $S_SECTORS
# Start offset of partition (bytes)
O_BYTES=$(( ${O_SECTORS} * 512 ))
# Size of partition (1k blocks, which mkdosfs expects)
S_BLOCKS=$(( ${S_SECTORS} / 2 ))
...
# Populate filesystem
mcopy -i ${TMPDIR}/image.dos syslinux.cfg ::syslinux.cfg || die "copy failed"
mcopy -i ${TMPDIR}/image.dos /usr/lib/syslinux/modules/bios/libcom32.c32 ::libcom32.c32 || die "copy failed"
mcopy -i ${TMPDIR}/image.dos /usr/lib/syslinux/modules/bios/mboot.c32 ::mboot.c32 || die "copy failed"
mcopy -i ${TMPDIR}/image.dos ${UNIKERNEL} ::unikernel.bin || die "copy failed"
